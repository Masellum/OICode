...
int main() {
    ...
    tree.build(1, 1, n);
    scanf("%d", &q);
    for (int i = 1, opt, x, formerSum; i <= q; ++i) {
        scanf("%d%d", &opt, &x);
        if (opt == 1) {
            tree.modifyInterval(1, 1, n, dfn[x], dfn[x] + size[x] - 1, 1);
        } else if (opt == 2) {
            do {
                tree.modifyInterval(1, 1, n, dfn[top[x]], dfn[x], 0);
                x = fa[top[x]];
            } while (x != 0);
        } else {
            printf("%d\n", tree.querySum(1, 1, n, dfn[x], dfn[x]));
        }
    }
    return 0;
}